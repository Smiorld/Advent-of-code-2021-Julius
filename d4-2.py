...
while len(bingo)>0 and index <len(data4):
    ...
    while winner_state[0]:
        ...
        last_score=counter*win_num
        print(last_score)
        bingo.pop(winner_state[1])
        marker.pop(winner_state[1])
        marker_counter.pop(winner_state[1])
        winner_state=check_bingo(marker_counter)
    index+=1
